...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (command == '1') {
      rotateServosTogether(servo1, servo2, 90, 5000);  // 4번, 5번 핀: 180 → 90
    } else if (command == '2') {
      rotateServosTogether(servo3, servo4, 90, 5000);  // 6번, 7번 핀: 180 → 90
    } else if (command == '3') {
      rotateServosTogether(servo5, servo6, 90, 5000);  // 8번, 9번 핀: 180 → 90
    }
  }
}
